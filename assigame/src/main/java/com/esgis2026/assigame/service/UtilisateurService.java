...
public class UtilisateurService {
    ...
    public Utilisateur registerPublicUser(RegisterUserRequest request) {
        ...
        utilisateur.setTelephone_utilisateur(request.getTelephone_utilisateur().trim());
        utilisateur.setMail_utilisateur(request.getMail_utilisateur().trim().toLowerCase());
        utilisateur.setLogin_utilisateur(request.getMail_utilisateur().trim().toLowerCase());
        utilisateur.setResidence_utilisateur(request.getResidence_utilisateur().trim());
        utilisateur.setPassword_utilisateur(
                passwordService.encodeIfNeeded(request.getPassword_utilisateur()));
    ...
    public Utilisateur updateUtilisateur(Utilisateur details, Long idUtilisateur) {
        ...
        if (details.getPassword_utilisateur() != null && !details.getPassword_utilisateur().isBlank()) {
            utiliateur.setPassword_utilisateur(
                    passwordService.encodeIfNeeded(details.getPassword_utilisateur()));
        }
    ...
    private TypeUtilisateur resolveSellerType() {
        return typeUtilisateurRepository.findAll().stream()
                .filter(type -> {
                    String label = type.getLibelle_type_utilisateur() != null
                            ? type.getLibelle_type_utilisateur().toLowerCase()
                            : "";
                    return label.contains("vendeur") || label.contains("seller");
                })
                .findFirst()
                .orElseThrow(() -> new RuntimeException("Type vendeur introuvable"));
    }
}
