...
| `productId` | `1` | ID produit pour les tests |
...
| Détail produit | `GET /produit/1` | `200` + objet produit |
| Image produit | `GET /produit/1/image` | `200` (binaire) ou `404` |
| Liste catégories | `GET /categorieproduit/list` | `200` |
...
**Inscription vendeur (public, sans token) :**
...
POST /users/add
{
  ...
  "login_utilisateur": "nouveau",
  "password_utilisateur": "motdepasse123",
  "residence_utilisateur": "Lomé",
  "type_utilisateur": { "id_type_utilisateur": 2 }
}
```

`id_type_utilisateur: 1` = Admin, `2` = Vendeur (après seed par défaut).
...
| `id_utilisateur` inconnu | `GET /users/list` en admin, ou lire `user.id_utilisateur` après login |
...
*Voir aussi [DOCUMENTATION.md](../DOCUMENTATION.md) section 9 (API REST).*
